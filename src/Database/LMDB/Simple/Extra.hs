...
foldDatabaseWithKey ::
  (Monoid m, Serialise k, Serialise v) =>
            ...
            case f key old of
              new@(Just newValue) -> marshalOut newValue $ \vval ->
                mdb_cursor_put' overwriteFlags cursor kval vval
                  >> return new
              Nothing ->
                mdb_cursor_del' defaultWriteFlags cursor
                  >> return old
