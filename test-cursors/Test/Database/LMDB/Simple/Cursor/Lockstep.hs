...
instance
  ...
  StateModel (Lockstep (CursorState k v))
  where
...
instance
  ...
  RunModel (Lockstep (CursorState k v)) (RealMonad k v ReadWrite)
  where
  perform _st = runCM
...
instance
  ...
  InLockstep (CursorState k v)
  where
  -- \| Model values
  data ModelValue (CursorState k v) a where
  ...
  -- \| Observable results
  data Observable (CursorState k v) a where
...
instance
  ...
  RunLockstep (CursorState k v) (RealMonad k v ReadWrite)
  where
  observeReal ::
...
arbitraryCursorAction ::
  forall k v.
  (Ord k, Eq v, Typeable k, Typeable v, Arbitrary k, Arbitrary v) =>
  ModelVarContext (CursorState k v) ->
  CursorState k v ->
  Gen (Any (LockstepAction (CursorState k v)))
...
shrinkCursorAction ::
  forall k v a.
  (Eq k, Eq v, Typeable k, Typeable v, Arbitrary k, Arbitrary v) =>
  ModelVarContext (CursorState k v) ->
  CursorState k v ->
  LockstepAction (CursorState k v) a ->
  [Any (LockstepAction (CursorState k v))]
