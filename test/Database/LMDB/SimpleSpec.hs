  ...
  describe "basic operations" $ do
    it "inserts and counts entries" $ \(env, db, _) ->
      transaction
        env
        ( do
            forM_ [1 .. 100] $ \i -> put db i (Just $ show i)
            size db
        )
        `shouldReturn` 100
    ...
    it "correctly serializes non-surrogate chars" $ \(env, db, _) ->
      let nonSurrogates = [minBound .. '\xD7FF'] ++ ['\xE000' .. maxBound]
       in transaction env (put db 0 (Just nonSurrogates) >> get db 0)
            `shouldReturn` Just nonSurrogates

    it "correctly serializes surrogate chars" $ \(env, db, _) ->
      let surrogates = ['\xD800' .. '\xDFFF']
       in transaction env (put db 0 (Just surrogates) >> get db 0)
            `shouldReturn` Just surrogates

  describe "transactions" $ do
    it "aborts" $ \(env, db, _) ->
      ( do
          transaction env $ put db 0 Nothing
    ...
    it "commits nested transactions" $ \(env, db, _) ->
      transaction
        env
        ( do
            nestTransaction (put db 3 $ Just "three")
            get db 3
        )
